...
local function redeemKey(key)
    -- Автоматически подготавливаем сессию (кеш ссылки), если пользователь не нажимал Get Key
    if cachedLink == "" then
        cacheLink()
    end
    ...
    if response and response.StatusCode == 200 then
        ...
        -- Если ключ уже активирован раннее в этой сессии / на устройствах, проверим через публичный эндпоинт или сохраним локально, 
        -- но стандартный PlatoBoost возвращает ошибку при повторном redeem. Попробуем мягкую проверку по файлу.
...
local function CreateGUI()
    ...
    -- Авто-логин (проверка сохраненного ключа без повторного вызова redeem, чтобы не ловить ошибку уже активированного ключа)
    if isfile and isfile(Config.KeyFileName) then
        local savedKey = readfile(Config.KeyFileName)
        if savedKey ~= "" then
            KeyInput.Text = savedKey
            Status.Text = "Found saved key, logging in..."
            task.spawn(function()
                -- Пропускаем полный повторный redeem для уже сохраненного ключа, либо проверяем мягко
                task.wait(0.3)
