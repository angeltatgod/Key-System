--[[
    ...
    Copyright (c) 2026 BULLY. All rights reserved.
    ================================================================
]]
local Config = {
    -- [1] PlatoBoost Settings
    ServiceId       = 31298, -- Your PlatoBoost Service ID
    PlatoSecret     = "ccb6ccf7-c1fc-44e4-87a5-cb27d499c218", -- Your PlatoBoost Secret Key

    -- [2] Anti-Bypass / Global Secret Variable
    Secret          = "BULLY1234", -- This makes the script ONLY run from the key script. Even if they copy the original obfuscated script to bypass the key, they won't be able to!
...
local function CreateGUI()
    ...
    -- Компактное окно со всеми исправлениями (без лишнего места снизу)
    local MainFrame = Instance.new("Frame", ScreenGui)
    MainFrame.Size = UDim2.new(0, 340, 0, 275)
    MainFrame.Position = UDim2.new(0.5, -170, 0.5, -137)
    ...
    local texts = {
        EN = {
            ...
            autoLogin = "Found saved key, logging in...",
            autoSuccess = "Auto-login success!",
            enterKey = "Enter a key!"
        },
        RU = {
            verify = "ПРОВЕРИТЬ",
            getKey = "ПОЛУЧИТЬ КЛЮЧ",
            buyVip = "⚡ ПОЛУЧИТЬ ДОСТУП НАВСЕГДА ⚡",
            copied = "ССЫЛКА СКОПИРОВАНА",
            statusWaiting = "Ожидание ввода...",
            placeholder = "Введите ключ...",
            toast = "Ссылка скопирована в буфер обмена!",
            verifying = "Проверка...",
            success = "Успешно! Загрузка...",
            gettingLink = "Получение ссылки...",
            linkCopied = "Ссылка скопирована!",
            autoLogin = "Найден сохраненный ключ...",
            autoSuccess = "Авто-вход выполнен!",
    ...
    -- Кнопки языка (RU и EN) в шапке
    local RuBtn = Instance.new("TextButton", Header)
    RuBtn.Size = UDim2.new(0, 28, 0, 24)
    RuBtn.Position = UDim2.new(1, -103, 0.5, -12)
    ...
    local EnBtn = Instance.new("TextButton", Header)
    EnBtn.Size = UDim2.new(0, 28, 0, 24)
    EnBtn.Position = UDim2.new(1, -135, 0.5, -12)
    ...
    local function updateLangButtonsVisual()
        if currentLang == "EN" then
            EnBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
            EnBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
            RuBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
            RuBtn.TextColor3 = Color3.fromRGB(150, 150, 150)
        else
            RuBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
            RuBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
            EnBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
            EnBtn.TextColor3 = Color3.fromRGB(150, 150, 150)
        end
    end
    ...
    -- Кликабельный промо-текст (Telegram) -> копирует ссылку ТГ с тостом
    local PromoText = Instance.new("TextButton", MainFrame)
    PromoText.Size = UDim2.new(0.9, 0, 0, 25)
    PromoText.Position = UDim2.new(0.05, 0, 0, 55)
    ...
    PromoText.AutoButtonColor = false

    -- Поле ввода ключа
    local KeyInput = Instance.new("TextBox", MainFrame)
    KeyInput.Size = UDim2.new(0.85, 0, 0, 36)
    KeyInput.Position = UDim2.new(0.075, 0, 0, 85)
    ...
    -- Кнопки (Get Key слева, Verify справа)
    local GetKeyBtn = Instance.new("TextButton", MainFrame)
    GetKeyBtn.Size = UDim2.new(0.415, 0, 0, 36)
    GetKeyBtn.Position = UDim2.new(0.075, 0, 0, 131)
    ...
    VerifyBtn.Size = UDim2.new(0.415, 0, 0, 36)
    VerifyBtn.Position = UDim2.new(0.51, 0, 0, 131)
    VerifyBtn.Text = texts[currentLang].verify
    ...
    -- Кнопка покупки доступа навсегда (снизу)
    local BuyBtn = Instance.new("TextButton", MainFrame)
    BuyBtn.Size = UDim2.new(0.85, 0, 0, 36)
    BuyBtn.Position = UDim2.new(0.075, 0, 0, 177)
    BuyBtn.Text = texts[currentLang].buyVip
    ...
    Status.Text = texts[currentLang].statusWaiting
    ...
    local function setLanguage(lang)
        ...
        GetKeyBtn.Text = texts[currentLang].getKey
        KeyInput.PlaceholderText = texts[currentLang].placeholder
        if Status.Text == texts[currentLang == "EN" and "RU" or "EN"].statusWaiting then
            Status.Text = texts[currentLang].statusWaiting
        end
    end

    EnBtn.MouseButton1Click:Connect(function() setLanguage("EN") end)
    RuBtn.MouseButton1Click:Connect(function() setLanguage("RU") end)

    -- Клик по надписи Telegram -> копирует ссылку на ТГ + тост
    PromoText.MouseButton1Click:Connect(function()
        fSetClipboard(Config.VipGroupURL)
        showToast(texts[currentLang].toast)
    end)

    -- Кнопка покупки вечного доступа -> копирует ссылку на ТГ + тост
    BuyBtn.MouseButton1Click:Connect(function()
        fSetClipboard(Config.VipGroupURL)
        BuyBtn.Text = texts[currentLang].copied
        showToast(texts[currentLang].toast)
    ...
    VerifyBtn.MouseButton1Click:Connect(function()
        ...
        Status.Text = texts[currentLang].verifying
        local success, msg = redeemKey(key)
        if success then
            Status.Text = texts[currentLang].success
    ...
    -- Кнопка Get Key (получение ссылки на ключ из PlatoBoost)
    GetKeyBtn.MouseButton1Click:Connect(function()
        Status.Text = texts[currentLang].gettingLink
